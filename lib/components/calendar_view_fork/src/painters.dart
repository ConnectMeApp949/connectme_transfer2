...
class HourLinePainterWithStartAndEndMinutes extends CustomPainter {
  /// Color of hour line
  final Color lineColor;
  ...
  /// Paints 24 hour lines.
  HourLinePainterWithStartAndEndMinutes({
    required this.lineColor,
    required this.lineHeight,
    required this.minuteHeight,
    required this.offset,
...
class HalfHourLinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    /// just need to add end minutes
    if  (endMinute >= 30) {
      // lg.t("[ENDMINUTE] endMinute >= 33 endhour " + endHour.toString());
      // lg.t("[ENDMINUTE] minuteheight ~  " + minuteHeight.toString() );
      final dy = (endHour - startHour) * minuteHeight * 60 ;

      final paint2 = Paint()
      ..color = lineColor
        // ..color = appPrimarySwatch
        ..strokeWidth = lineHeight;
        canvas.drawLine(Offset(offset, dy), Offset(size.width, dy), paint2);
