...
class _TimeLineState extends State<TimeLine> {
  // late Timer _timer;
  late final TimeOfDay _currentTime = TimeOfDay.now();
  ...
  // bool _isWithinRange(int hour, int minute) {
  //   final startTotal = widget.startHour * 60 + widget.startMinute;
  //   final endTotal = widget.endHour * 60 + widget.endMinute;
  //   final current = hour * 60 + minute;
  //   return current >= startTotal && current <= endTotal;
  // }

  double _calculateTop(int hour, int minute) {
    final minutesFromStart = (hour - widget.startHour) * 60 +
        (minute - widget.startMinute);
    return (minutesFromStart / 60) * widget.hourHeight -
        widget.timeLineOffset;
  }

  double _calculateBottom(int hour, int minute) {
    final minutesFromEnd = (widget.endHour - hour) * 60 +
        (widget.endMinute - minute);
    return (minutesFromEnd / 60) * widget.hourHeight +
        widget.timeLineOffset;
  }

  List<int> _getValidMinutesForHour(int hour) {
    if (hour == widget.startHour && hour == widget.endHour) {
      // same hour (e.g., 08:15 to 08:45)
      return [0, 15, 30, 45]
          .where((m) => m >= widget.startMinute && m <= widget.endMinute)
          .toList();
    } else if (hour == widget.startHour) {
      return [0, 15, 30, 45]
          .where((m) => m >= widget.startMinute)
          .toList();
    } else if (hour == widget.endHour) {
      return [0, 15, 30, 45]
          .where((m) => m <= widget.endMinute)
          .toList();
    } else {
      return [0, 15, 30, 45];
    }
  ...
  Widget build(BuildContext context) {
    ...
    //       //     if (_isWithinRange(hour, minute))
    //       //       _timelinePositioned(
    //       //         topPosition: _calculateTop(hour, minute),
    //       //         bottomPosition: _calculateBottom(hour, minute),
    //       //         hour: hour,
    //       //         minutes: minute,
    //       //       ),
    //
    //       for (int hour = widget.startHour; hour <= widget.endHour; hour++)
    //         for (int minute in _getValidMinutesForHour(hour))
    //           _timelinePositioned(
    //             topPosition: _calculateTop(hour, minute),
    //             bottomPosition: _calculateBottom(hour, minute + 15),
    //             hour: hour,
    //             minutes: minute,
    //           ),
    //
...
class EventGenerator<T extends Object?> extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // TOODOO: Use SizedBox If possible.
    return Container(
      height: height,
      width: width,
      child: Stack(
