      ...
      child: Stack(
        children: [
          for (int i = 0; i < slots; i++)
            Positioned(
              top: (heightPerSlot * i ) - timeLineOffset,
              left: 0,
              right: 0,
              bottom: height - (heightPerSlot * (i + 1)) + timeLineOffset,
...
class DefaultPressDetectorWithMinutes extends StatelessWidget {
  /// default press detector builder used in week and day view
  const DefaultPressDetectorWithMinutes({
    super.key,
    required this.date,
    required this.height,
    required this.width,
  ...
  DateTime getSlotDateTime(int slot) => DateTime(
    ...
    date.day,
    0,
    (minuteSlotSize.minutes * slot) + (startHour * 60) + (startMinute),
  );
}
