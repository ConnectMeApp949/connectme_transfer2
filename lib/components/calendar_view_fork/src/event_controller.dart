...
class EventController<T extends Object?> extends ChangeNotifier {
  ...
  // TOODOO: change the type from List<CalendarEventData>
...
class CalendarData<T extends Object?> {
  ...
  void addFullDayEvent(CalendarEventData<T> event) {
    // TOODOO: add separate logic for adding full day event and ranging event.
  ...
  void removeWhere(TestPredicate<CalendarEventData<T>> test) {
    final predicates = <CalendarEventData<T>, bool>{};

    bool wrappedPredicate(CalendarEventData<T> event) {
      return predicates[event] = test(event);
    }
    ...
    _eventList.removeWhere((event) => predicates[event] ?? false);
  }
  ...
  List<CalendarEventData<T>> getEventsOnDay(DateTime date,
    ...
    // TOODOO(Shubham): Add recurrence support for ranging events
