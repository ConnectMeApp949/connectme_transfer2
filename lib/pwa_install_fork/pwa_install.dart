...
import 'package:connectme_app/pwa_install_fork/js_stub.dart' if (dart.library.js) 'package:js/js.dart';

///library pwa_install_fork;
...
class PWAInstall {
  ...
  void promptInstall_() {
    lg.t("[PWA INSTALL] [promptInstall_] called");
    if (hasPrompt) {
      promptInstall();
    } else {
  ...
  void setup({Function? installCallback}) {
    ...
    lg.t("[PWA INSTALL] [setup call get launch mode] called");
    getLaunchMode_();
    onAppInstalled = installCallback;
    lg.t("[PWA INSTALL] [setup call get launch mode] callback set");
  }
}
